...
@endsection
@section('page-script')
@vite('resources/assets/js/app-artist-exhibition.js')
@endsection
@php
...
@section('content')
                ...
                <div class="user-profile-header d-flex flex-column flex-lg-row text-sm-start text-center mb-5">
                    ...
                    <div class="flex-grow-1 mt-3 mt-lg-5">
                        <div
                            ...
                            <button type="button" class="btn btn-primary btn-add-exhibition">
                                <i class="ti ti-plus me-2"></i>{{ trans('Add New Exhibition') }}
                            </button>
                        </div>
        ...
        <div class="col-md-12">
            <div class="nav-align-top">
                <ul class="nav nav-pills flex-column flex-sm-row mb-6 gap-2 gap-lg-0">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="{{ url('artist/profile-exhibition') }}"><i
        ...
        @forelse ($exhibitions as $exhibition)
            <div class="col-xl-4 col-lg-6 col-md-6 mb-4">
                <div class="card h-100">
                    <!-- Status Ribbon -->
                    <span class="badge bg-{{ $exhibition->status_color }} position-absolute top-0 end-0 m-2 z-3" style="z-index: 1;">
                        {{ $exhibition->status_text }}
                    </span>
                    <!-- Exhibition Image -->
                    @if ($exhibition->image)
                        <img src="{{ Storage::url($exhibition->image) }}" class="card-img-top"
                            alt="{{ $exhibition->name }}" style="height: 180px; object-fit: cover;">
                    @else
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            <div>
                                <h5 class="card-title mb-1">{{ $exhibition->name }}</h5>
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    <li>
                                        @php
                                            $data = json_encode([
                                                ...
                                                'start_date' => $exhibition->start_date->format('Y-m-d'),
                                                'end_date' => $exhibition->end_date->format('Y-m-d'),
                                                'location' => $exhibition->location,
                                                'description' => $exhibition->description,
                                                'image' => $exhibition->image,
                                                'image_url' => $exhibition->image
                                                    ? Storage::url($exhibition->image)
                                                    : null,
                                            ]);

                                        @endphp
                                        <a class="dropdown-item btn-edit-exhibition" href="#"
                                            data-id="{{ $exhibition->id }}"data-exhibition="{{ $data }}">
                                            <i class="ti ti-pencil me-2"></i>{{ trans('Edit Exhibition') }} 
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item" href="#" data-bs-toggle="modal"
                                            data-bs-target="#exhibitionModal{{ $exhibition->id }}">
                                    ...
                                    <li>
                                        <form class="delete-confirmation" method="POST" action="{{ route('artists.exhibitions.destroy', [$artist->id, $exhibition->id]) }}">
                                            @csrf
                                            @method('DELETE')
                        ...
                        <p class="card-text text-muted mb-4">{{ $exhibition->short_description }}</p>

                        <div class="d-flex justify-content-between align-items-center border-top pt-3">
                            <div>
                                <small class="text-muted">{{ trans('Start Date') }} </small>
                                <p class="mb-0 fw-semibold">{{ $exhibition->start_date->format('Y-m-d') }}</p>
                            </div>
                            <div class="text-end">
                                <small class="text-muted">{{ trans('End Date') }} </small>
                                <p class="mb-0 fw-semibold">{{ @$exhibition->end_date->format('Y-m-d') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal fade" id="exhibitionModal{{ $exhibition->id }}" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">{{ $exhibition->name }}</h5>
                        ...
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-md-6 mb-4">
                                    @if ($exhibition->image)
                                        <img src="{{ Storage::url($exhibition->image) }}" class="img-fluid rounded"
                                            alt="{{ $exhibition->name }}">
                                    @endif
                                ...
                                <div class="col-md-6">
                                    <h6 class="mb-3">{{ trans('Exhibition Details') }} </h6>
    ...
    <!-- Add this at the bottom of your exhibitions/index.blade.php -->
    <div class="modal fade" id="exhibitionModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTitle">{{ trans('Add New Exhibition') }}</h5>
                ...
                <form id="exhibitionForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            <div class="col-md-12">
                                <label class="form-label">{{ trans('Exhibition Name') }} <span class="text-danger">*</span></label>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('.btn-add-exhibition').addEventListener('click', function() {
                form.reset();
                modalTitle.textContent = 'Add New Exhibition';
                form.action = "{{ route('artists.exhibitions.store', $artist) }}";
            ...
            document.querySelectorAll('.btn-edit-exhibition').forEach(button => {
                button.addEventListener('click', function() {
                    const exhibitionId = this.dataset.id;
                    console.log(exhibitionId);
                    const exhibition = JSON.parse(this.dataset.exhibition);
                    ...
                    form.action =
                        "{{ route('artists.exhibitions.update', [$artist, 'exhibition_id']) }}"
                        .replace('exhibition_id', exhibitionId);
                    ...
                    if (exhibition.image_url) {
                        imagePreview.innerHTML = `
                    <p>Current Image:</p>
                    <img src="${exhibition.image_url}" class="img-thumbnail" width="150">
                    <input type="hidden" name="current_image" value="${exhibition.image}">
                `;
                    } else {
                        imagePreview.innerHTML = '';
                    }

                    modalTitle.textContent = 'Edit Exhibition';
                    modal.show();
                });
            });
